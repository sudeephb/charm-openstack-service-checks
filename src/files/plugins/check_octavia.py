...
def check_loadbalancers(connection):
    ...
    bad_lbs = [(
        ...
        ) for lb in lb_enabled if lb.provisioning_status != 'ACTIVE']

    # raise WARNING if operating_status is not ONLINE
    bad_lbs += [(
        NAGIOS_STATUS_CRITICAL,
        'loadbalancer {} operating_status is {}'.format(
            lb.id, lb.operating_status)
        ) for lb in lb_enabled if lb.operating_status != 'ONLINE']
    ...
    bad_lbs += [(
        ...
        ) for lb in lb_all if not lb.is_admin_state_up]
...
def check_amphorae(connection):
    ...
    bad_amp = [(
        ...
        ) for item in items if item['status'] in bad_status_list]
