...
class ActionFailed(Exception):
    ...
    def __init__(self, action):
        ...
        params = {
            key: getattr(action, key, "<not-set>")
            for key in [
                "name",
                "parameters",
                "receiver",
        ...
        message = (
            ...
            "completed={completed})".format(**params)
        )
...
class Agent:
    ...
    def status(self, status):
        return self.unit.workload_status == status and self.unit.agent_status == "idle"
...
async def deploy_openstack(model):
    ...
    for app in apps.keys():
        if app in model.applications:
            ...
            is_deployed = True
            continue
        app_deploy = await model.deploy(
            "cs:{}".format(app), series="bionic", application_name=app, num_units=1
        )
        if app not in blocking_apps:
    ...
    await model.add_relation(
        "{}:shared-db".format(apps["keystone"]),
    ...
    for app in "neutron-api nova-cloud-controller".split():
        ...
        await model.add_relation(
            "{}:amqp".format(apps[app]), "{}:amqp".format(apps["rabbitmq-server"])
        )
        await model.add_relation(
            "{}:shared-db".format(apps[app]),
            "{}:shared-db".format(apps["percona-cluster"]),
        )
        await model.add_relation(
            "{}:identity-service".format(apps[app]),
            "{}:identity-service".format(apps["keystone"]),
        )
    await model.add_relation(
        "{}:identity-service".format(apps["ceph-radosgw"]),
        "{}:identity-service".format(apps["keystone"]),
    )
    yield active_apps
...
async def deploy_app(request, deploy_openstack, model):
    await model.block_until(
        lambda: all([app.status == "active" for app in deploy_openstack]), timeout=1200
    )
    series = request.param
    ...
    await model.deploy(
        "cs:nrpe", series=series, application_name=apps["nrpe"], num_units=0
    )
    osc_app = await model.deploy(
        os.path.join(CHARM_BUILD_DIR, "openstack-service-checks"),
        series=series,
...
async def test_openstackservicechecks_verify_default_nrpe_checks(
...
async def test_openstackservicechecks_update_endpoint(deploy_app, model, file_stat):
    ...
    await model.block_until(
        lambda: rgw.units[0].agent_status == "idle", timeout=600, wait_period=1
    )
    await model.block_until(
        lambda: keystone.status == "active" and kst_unit.agent_status == "idle",
        timeout=600,
        wait_period=1,
    )
    await model.block_until(
    ...
    for _ in range(10):
        ...
        for endpoint in "admin internal public".split():
            ...
            assert result["Code"] == "0", "Error {}: {}".format(
...
async def test_openstackservicechecks_remove_endpoint_checks(
    deploy_app, model, file_stat
):
    unit = unit_from(model, deploy_app.name)
    endpoint_checks_config = {
...
async def test_openstackservicechecks_disable_check_neutron_agents(
    deploy_app, model, file_stat
):
    unit = unit_from(model, deploy_app.name)
...
async def test_openstackservicechecks_invalid_keystone_workload_status(
    model, deploy_app
):
